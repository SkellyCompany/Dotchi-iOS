...
extension DotchiRepository: DotchiRepositoryProtocol {
    func getDotchi(id: String, completionHandler: @escaping DotchiResultCallback) {
        httpService.get(
            response: DotchiDTO.self,
            endpointUrl: Endpoint.dotchi(dotchiId: id).getUrlString(with: environment)) { result in
            switch result {
    ...
    func observeStatistics(dotchiId: String, completionHandler: @escaping StatisticsResultCallback) {
        socketService.on(.custom(name: Endpoint.updatedStatistics(dotchiId: dotchiId).getEventString()),
                         with: DotchiStatisticsDTO.self,
                         in: .json,
    ...
    func observeMetrics(dotchiId: String, completionHandler: @escaping MetricsResultCallback) {
        socketService.on(.custom(name: Endpoint.updatedMetrics(dotchiId: dotchiId).getEventString()),
                         with: DotchiMetricsDTO.self,
                         in: .json, completionHandler: { result in
                            switch result {
