...
extension App {
    func setupLanguage() {
        ...
        languageController.switchApplicationLanguage(
            to: preferences.applicationLanguage.code,
            localizableBundle: &L10n.bundle)
    }
    ...
    func setupTheme(in windows: [UIWindow]) {
        guard let themeController = themeController,
              let userPreferencesRepository = userPreferencesRepository else { return }
        themeController.switchActiveTheme(
            to: userPreferencesRepository.getUserPreferences().activeTheme.interfaceStyle,
            in: windows)
    }
    ...
    func launch(in window: UIWindow) {
        ...
        group.notify(queue: queue) {
            ...
            let dotchiTabBarItem = UITabBarItem(
                title: "Browse",
                image: UIImage(systemName: "pawprint"),
                selectedImage: UIImage(systemName: "pawprint.fill"))
            let logsTabBarItem = UITabBarItem(
                title: "Logs",
                image: UIImage(systemName: "doc.badge.gearshape"),
                selectedImage: UIImage(systemName: "doc.badge.gearshape.fill"))

            DispatchQueue.main.async {
                self.router?.route(
                    to: .dotchi(model: DotchiRouteModel(dotchi: dotchi)),
                    style: .root(window: window, transition: .flipToLeft),
                    options: [.wrapInTabBar(tabBarItem: dotchiTabBarItem,
                                            siblings: [.logs(model: LogsRouteModel(logs: logs)): logsTabBarItem])])
            }
        }
    }
